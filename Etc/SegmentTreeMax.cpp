...
void updateSegmentTree(int idx, int start, int v) {
    ...
    if (seg[(idx + start) / 2] < v) {
        updateSegmentTree(idx / 2, start / 2, v);
    }
    seg[idx + start] = v;
}
...
int querySegmentTree(int left, int right, int x, int y, int node) {
    ...
    ans = 0;
    ans = max(ans, querySegmentTree(left, mid, x, min(y, mid), node * 2));
    ans = max(ans, querySegmentTree(mid + 1, right, max(x, mid + 1), y, node * 2 + 1));
...
int main(int argc, const char * argv[]) {
    ...
    updateSegmentTree(0, start, 8);
    
    printf("%d\n", querySegmentTree(0, start - 1, 0, 1, 1));
    printf("%d\n", querySegmentTree(0, start - 1, 0, 2, 1));
    printf("%d\n", querySegmentTree(0, start - 1, 0, 9, 1));
    
    updateSegmentTree(9, start, 37);
