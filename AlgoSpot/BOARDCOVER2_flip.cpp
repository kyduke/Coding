...
void preCalculateBlocks(char buffer[10][16], int R, int C) {
	...
	for (j =0; j < R; j++) {
		...
		temp[C] = '\0';
		blocks[2].board[j] = row2mask(temp, false);
	}

	//rotate 180, flip horizontal O
	blocks[3].w = C;
	blocks[3].h = R;
	for (j = R - 1; j >= 0; j--) {
		blocks[3].board[R - 1 - j] = row2mask(buffer[j], false);
	}

	//rotate 90, flip horizontal X
	blocks[4].w = R;
	blocks[4].h = C;
	for (i = C - 1; i >= 0; i--) {
		for (j = 0; j < R; j++) {
		...
		temp[R] = '\0';
		blocks[4].board[C - 1 - i] = row2mask(temp, false);
	}

	//rotate 270, flip horizontal X
	blocks[5].w = R;
	blocks[5].h = C;
	for (i = 0; i < C; i++) {
		for (j = R - 1; j >= 0; j--) {
		...
		temp[R] = '\0';
		blocks[5].board[i] = row2mask(temp, false);
	}

	//rotate 90, flip horizontal O
	blocks[6].w = R;
	blocks[6].h = C;
	for (i = 0; i < C; i++) {
		for (j = 0; j < R; j++) {
		...
		temp[R] = '\0';
		blocks[6].board[i] = row2mask(temp, false);
	}

	//rotate 270, flip horizontal O
	blocks[7].w = R;
	blocks[7].h = C;
	for (i = C - 1; i >= 0; i--) {
		for (j = R - 1; j >= 0; j--) {
		...
		temp[R] = '\0';
		blocks[7].board[C - 1 - i] = row2mask(temp, false);
	}
